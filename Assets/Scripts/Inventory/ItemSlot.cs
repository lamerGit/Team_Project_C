...
public class ItemSlot
{
    ...
    {
        bool result = false;
        IEquipArtifact equipItem = SlotItemData as IEquipArtifact;  // 이 슬롯의 아이템이 장비 가능한 아이템인지 확인
        if (equipItem != null)
        {
            // 아이템은 장비가능하다.

            ItemData_Artifact artifactData = SlotItemData as ItemData_Artifact;   // 아이템 데이터 따로 보관
